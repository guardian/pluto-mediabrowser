import {VidispineShape, VidispineShapeIF} from "../../../app/vidispine/shape/VidispineShape";
import fs from "fs";
        ...
        fs.readFile(__dirname + "/example-shape.json", (err, data ) => {
            if(err) {
                ...
                try {
                    ...
                    expect(shape.containerComponent?.timeCodeTimeBase).toEqual({"numerator":1,"denominator":25});
        ...
        fs.readFile(__dirname + "/example-broken-shape.json", (err, data ) => {
            if(err) {
                ...
                try {
                    ...
                    if(err instanceof VError && err.message.includes("value.containerComponent.id is missing")) {
                        done();
                    } else {
                        console.error("Received unexpected error. Expected a VError containing a message that containerComponent.id is missing.")
                        done.fail(err);
                    }
                }
            }
