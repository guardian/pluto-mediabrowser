...
const EmbeddablePlayer:React.FC<RouteComponentProps> = (props) => {
            ...
            if(maybeId) {
                ...
                loadItemMeta(vidispineContext.baseUrl, maybeId)
                    ...
                    .catch(err => {
                        setLoading(false);
                        setLastError(err);
                        if (err.includes("retrying")) window.setTimeout(()=>loadItemMeta(vidispineContext.baseUrl, maybeId), 3000);  //try again in 3 seconds
                    })
            } else {
                console.error("Can't load item because no onlineId was present. Request was ", params);
            }
        } else {
    ...
    return <div id="mediabrowser-embed">
        ...
        { lastError ? <Typography variant="caption">{lastError}</Typography> : undefined }
    </div>
}
